...
class local_ombiel_webservices_testcase extends advanced_testcase {
    ...
    public function test_get_course_sections() {
     ...
     public function test_get_cm_assignment() {
        ...
        $this->assertArrayNotHasKey('onlinesubmission',$assignmentresult);
        $this->assertArrayNotHasKey('filesubmissions', $assignmentresult);
        $this->assertArrayNotHasKey('comments', $assignmentresult);
        $this->assertEquals(true, $assignmentresult['cansubmit']);
        $this->assertArrayNotHasKey('grade',$assignmentresult);
        $this->assertArrayNotHasKey('gradedon',$assignmentresult);
         ...
         *  that have been submitted but not marked
         */

        /**
         * Create a second submission attempt to check that we pick up the correct one.
         */
        ...
        // Create an online submission.

        $onlinetextsubmission = new stdClass();
        $onlinetextsubmission->onlinetext = '<p>online test text</p><img src="@@PLUGINFILE@@/_dummy.jpg" height="20" width="20" />';
        $onlinetextsubmission->onlineformat = 1;
        $onlinetextsubmission->submission = $submission->id;
        $onlinetextsubmission->assignment = $assignmentinstance->id;
        $onlinetextsubmission->id = $DB->insert_record('assignsubmission_onlinetext', $onlinetextsubmission);

        $cm = $DB->get_record('course_modules', array('id'=>$assignmentinstance->cmid));
        $assign = new assign($cmcontext, $cm, $course1);
        // Now two file submissions.
        ...
        $sourcefile = $CFG->dirroot.'/README.txt';
        ...
        $fs->create_file_from_pathname($pdfsubmission2, $sourcefile);
        ...
        $comment1->contextid = $cmcontext->id;
        $comment1->commentarea = 'submission_comments';
        $comment1->itemid = $submission->id;
        $comment1->content = 'First comment';
        $comment1->format = 0;
        $comment1->userid = $student->id;
        $comment1->timecreated = time();
        $DB->insert_record('comments', $comment1);

        $comment1 = new stdClass();
        $comment1->contextid = $cmcontext->id;
        $comment1->commentarea = 'submission_comments';
        $comment1->itemid = $submission->id;
        $comment1->content = 'Second comment';
        $comment1->format = 0;
        $comment1->userid = $student->id;
        $comment1->timecreated = time();
        $DB->insert_record('comments', $comment1);
        ...
        /**
         * Create user flag record for student
         */
        $userflag = new stdClass();
        $userflag->assignment = $assignmentinstance->id;
        $userflag->userid = $student->id;
        $userflag->locked = 0;
        $userflag->mailed = 0;
        $userflag->extensionduedate = 0;
        $userflag->workflowstate = 'inmarking'; // Should prevent grade and feedback showing
        $userflag->allocatedmarker = $teacher->id;
        $userflag->id = $DB->insert_record('assign_user_flags', $userflag);
        ...
        $this->assertEquals('<div class="no-overflow"><p>online test text</p><img src="'.$CFG->wwwroot.
                '/webservice/pluginfile.php/'.$cmcontext->id
                .'/assignsubmission_onlinetext/submissions_onlinetext/'.$submission->id.
                '/_dummy.jpg" height="20" width="20" alt="_dummy.jpg" /></div>',
                $assignmentresult['onlinesubmission']);
        $this->assertEquals(2, count($assignmentresult['filesubmissions']));

        $this->assertEquals($CFG->wwwroot.'/webservice/pluginfile.php/'.
            $cmcontext->id.'/assignsubmission_file/submission_files/'.$submission->id
            .'/submission.pdf?forcedownload=1',
            $assignmentresult['filesubmissions'][0]['link']);
        $this->assertEquals('submission.pdf',
            $assignmentresult['filesubmissions'][0]['name']);
        $this->assertEquals('submission.pdf',
            $assignmentresult['filesubmissions'][0]['title']);

        $this->assertEquals($CFG->wwwroot.'/webservice/pluginfile.php/'.
            $cmcontext->id.'/assignsubmission_file/submission_files/'.$submission->id
            .'/submission2.pdf?forcedownload=1',
            $assignmentresult['filesubmissions'][1]['link']);
        $this->assertEquals('submission2.pdf',
            $assignmentresult['filesubmissions'][1]['name']);
        $this->assertEquals('submission2.pdf',
            $assignmentresult['filesubmissions'][1]['title']);


        $this->assertEquals(2, count($assignmentresult['comments']));
        $this->assertEquals('<div class="no-overflow"><div class="text_to_html">First comment</div></div>',
            $assignmentresult['comments'][0]['content']);
        $this->assertEquals('<div class="no-overflow"><div class="text_to_html">Second comment</div></div>',
            $assignmentresult['comments'][1]['content']);
        ...
        /**
         * Lock above assignment
         */

        $userflag->locked = 1;
        $DB->update_record('assign_user_flags', $userflag);
        /**
         * Grade above assignment
         */

        $this->setUser($teacher);
        $grade = $assign->get_user_grade($student->id, true);

        $grade->grade = '50.01000001';
        $DB->update_record('assign_grades', $grade);
        $data = new stdClass();
        $data->grade = '50.01000001';
        $data->assignfeedbackcomments_editor = array('text'=>'<p>online feedback</p>',
                                         'format'=>FORMAT_MOODLE);
        $commplugin = $assign->get_feedback_plugin_by_type('comments');

        $commplugin->save($grade, $data);
        $gradedon = time();

        /*
         * Add two files for feedback
         */

        $fs = get_file_storage();
        $sourcefile = $CFG->dirroot.'/README.txt';

        $feedbackfile = (object) array(
            'contextid' => $cmcontext->id,
        ...
        $fs->create_file_from_pathname($feedbackfile, $sourcefile);

        $data = new stdClass();
        $data->grade = '50.01000001';
        $data->files_3_filemanager = '';
        $fileplugin = $assign->get_feedback_plugin_by_type('file');
        $fileplugin->save($grade, $data);
        ...
        /**
         * Grade and feedback shouldn't show because of workflow status
         */
        ...
        $this->assertEquals('<div class="no-overflow"><p>online test text</p><img src="'.$CFG->wwwroot.
                '/webservice/pluginfile.php/'.$cmcontext->id
                .'/assignsubmission_onlinetext/submissions_onlinetext/'.
                $submission->id.'/_dummy.jpg" height="20" width="20" alt="_dummy.jpg" /></div>',
                $assignmentresult['onlinesubmission']);
        $this->assertEquals(2, count($assignmentresult['filesubmissions']));
        ...
        $this->assertEquals(false, $assignmentresult['cansubmit']);

        /**
         * Workflow not in Moodle 2.5
         */
        ...
        if ($CFG->version >= 2013111800) { // Moodle 2.6
            ...
            /**
             * Change workflow status and check grade and feedback
             */

            $userflag->workflowstate = 'released'; // Should now show grade and feedback showing
            $userflag->id = $DB->update_record('assign_user_flags', $userflag);

            $assignmentresult = local_ombiel_webservices::get_cm_assignment($assignmentinstance->cmid);
            $assignmentresult = external_api::clean_returnvalue(local_ombiel_webservices::get_cm_assignment_returns(), $assignmentresult);

        }
        $this->assertEquals('50.01', $assignmentresult['grade']);
        $this->assertEquals(round($gradedon,-1), round($assignmentresult['gradedon'],-1));
        $this->assertEquals(fullname($teacher)  , $assignmentresult['gradedby']);
        $this->assertEquals('<p>online feedback</p>', $assignmentresult['feedback']);
        $this->assertEquals(2, count($assignmentresult['filefeedback']));

        $this->assertEquals($CFG->wwwroot.'/webservice/pluginfile.php/'.
             $cmcontext->id.'/assignfeedback_file/feedback_files/'.$grade->id
            .'/feedback1.pdf?forcedownload=1',
            $assignmentresult['filefeedback'][0]['link']);
        $this->assertEquals('feedback1.pdf',
            $assignmentresult['filefeedback'][0]['name']);
        $this->assertEquals('feedback1.pdf',
            $assignmentresult['filefeedback'][0]['title']);

        $this->assertEquals($CFG->wwwroot.'/webservice/pluginfile.php/'.
             $cmcontext->id.'/assignfeedback_file/feedback_files/'.$grade->id
            .'/feedback2.pdf?forcedownload=1',
            $assignmentresult['filefeedback'][1]['link']);
        $this->assertEquals('feedback2.pdf',
            $assignmentresult['filefeedback'][1]['name']);
        $this->assertEquals('feedback2.pdf',
            $assignmentresult['filefeedback'][1]['title']);
        ...
        $assignmentresult = local_ombiel_webservices::get_cm_assignment($assignmentinstance2->cmid);
        $assignmentresult = external_api::clean_returnvalue(local_ombiel_webservices::get_cm_assignment_returns(), $assignmentresult);

        $this->assertEquals(false, $assignmentresult['cansubmit']);
        /**
         * assignment open for submissions
         */
        $options = array(
            'course'=>$course1->id,
        ...
        $assignmentinstance3 = $assignmentgenerator->create_instance($options);

        $assignmentresult = local_ombiel_webservices::get_cm_assignment($assignmentinstance3->cmid);
        $assignmentresult = external_api::clean_returnvalue(local_ombiel_webservices::get_cm_assignment_returns(), $assignmentresult);

        $this->assertEquals(true, $assignmentresult['cansubmit']);
        /**
         * assignment closed for submissions
         */
        $options = array(
            'course'=>$course1->id,
        ...
        $assignmentinstance4 = $assignmentgenerator->create_instance($options);

        $assignmentresult = local_ombiel_webservices::get_cm_assignment($assignmentinstance4->cmid);
        $assignmentresult = external_api::clean_returnvalue(local_ombiel_webservices::get_cm_assignment_returns(), $assignmentresult);
        ...
        $userflag = new stdClass();
        $userflag->assignment = $assignmentinstance5->id;
        $userflag->userid = $student->id;
        $userflag->locked = 0;
        $userflag->mailed = 0;
        $userflag->extensionduedate =  $tomorrow;
        $userflag->workflowstate = 'inmarking';
        $userflag->allocatedmarker = $teacher->id;
        $userflag->id = $DB->insert_record('assign_user_flags', $userflag);

        $assignmentresult = local_ombiel_webservices::get_cm_assignment($assignmentinstance5->cmid);
        $assignmentresult = external_api::clean_returnvalue(local_ombiel_webservices::get_cm_assignment_returns(), $assignmentresult);
        $this->assertEquals(true, $assignmentresult['cansubmit']);
